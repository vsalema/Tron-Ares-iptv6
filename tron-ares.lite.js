...
function ensureRadioOverlayLayer() {
  ...
  // LITE: pas de blur plein écran (meilleur INP)
...
function showRadioOverlayInPlayer() {
  ...
  // Affichage immédiat (paint rapide)
  ...
  // Charge l’iframe après le premier paint => réduit le "Presentation delay" sur INP
  requestAnimationFrame(() => {
    setTimeout(() => {
      const mustLoad = (!iframe.src || iframe.src === 'about:blank' || iframe.dataset.loaded !== '1');
      if (mustLoad) {
        iframe.src = url;
      ...
      if (!iframe.dataset.pmBound) {
        ...
        iframe.addEventListener('load', () => {
          ...
          // Si l’utilisateur a demandé la radio, on relance RADIO ALFA après le load
          if (radioPlaying) {
...
if (miniRadioEl && radioPlayBtn) {
  // LITE: évite les déclenchements clavier en rafale (Space/Enter maintenus)
  radioPlayBtn.addEventListener('keydown', (e) => {
    if ((e.code === 'Space' || e.code === 'Enter') && e.repeat) {
      e.preventDefault();
    }
  }, { passive: false });

  // LITE: anti-double-clic / anti-repeat click (réduit la charge de paint)
  let lastRadioToggleAt = 0;

  radioPlayBtn.addEventListener('click', () => {
    const now = performance.now();
    if (now - lastRadioToggleAt < 320) return;
